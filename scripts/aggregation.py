...
def buildAggs(path, name):
	...
	dbName =  dbName + ".adb"
	for cRatio in [0.4,0.5,0.6,0.7,0.8,0.9]:
...
def searchAggregate( aggregate, queries, threshold):
	logName = "{}_{}_{}".format(aggregate.split(".")[0], queries.split("/")[-1], threshold)
	command = "./Ammolite aggsearch -c {} -q {} -t {} --target DEV-TEST 2>&1 | tee {}".format(aggregate, queries, threshold, logName)
	print( "Running: "+ command)
	call( command, shell=True)
...
def searchAggs( folderName, queries):
	...
	for f in files:
		for thresh in [0.4, 0.5, 0.6, 0.7, 0.8, 0.9]:
			...
			searchAggregate(fName, queries, thresh)
